...
contract MockSynassetsTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sSYNASSETSQueue; // Delays change to sSYNASSETS address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSYNASSETS).balanceOf(msg.sender); // Can only borrow against sSYNASSETS held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithSYNASSETS(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(SYNASSETS).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SSYNASSETS) {
      // 9
